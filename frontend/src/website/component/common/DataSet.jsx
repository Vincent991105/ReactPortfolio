...
import typeMapping from '../../js/typeMapping.js';

function DataSet({ selectSensorData, lineActive, openChart, type, q1, q2, q3, fitCurve, time, value }) {
  ...
  return (
    <div className="DataSet" onClick={openChart ? (() => type && openChart(type)) : undefined}>
      ...
      <div className="Dashboard"
        ...
        //   }
        >
          <h4>{typeMapping[type].name ? typeMapping[type].name : type}</h4>
          <h4>{Number(value).toFixed(2)} <span>{typeMapping[type].unit}</span></h4>
        </div>
