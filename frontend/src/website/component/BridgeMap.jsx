...
function BridgeMap() {
  ...
  const { list, data, earthSelect, colormode } = useSelector((state) => state.ProjectBridgeData);
  ...
  const alertCustomIcon = (id, healthErrors, connectionErrors) => {
    ...
    const fillColor = id === data.id ? "#e95098" : "#ff69b4"; // 选中时深粉色，默认浅粉色
    ...
    svg.innerHTML = `
      <defs>
        <radialGradient id="earthquake-gradient" cx="50%" cy="50%" r="50%">
          ...
          <stop offset="50%" style="stop-color: red; stop-opacity: 0.4" />
    ...
    <div className="BridgeMap">
      ...
      >
        ...
        {earthSelect && (
          <>
            <Circle
              center={[earthSelect.latitude, earthSelect.longitude]}
              radius={Math.pow(10, (earthSelect.magnitudeValue - 4)) * 50 * 1000} // 擴散範圍，假設每個震級對應 1000 公里
              color="hidden"
              fillColor="url(#earthquake-gradient)"
              fillOpacity={1}
            ></Circle>
            <Marker position={[earthSelect.latitude, earthSelect.longitude]} icon={EarthquakeIcon()} />
          </>
        )}
