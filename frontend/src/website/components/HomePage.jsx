...
import { AiFillFileUnknown, AiFillHome } from "react-icons/ai";
import {
  Table,
  TableContainer,
  TableHead,
...
const HomePage = () => {
  const { dataTime, structuresList, selectBridge, fetchStructures } = useContext(BridgeContext);
  ...
  useEffect(() => {
    const iconFilterList = structuresList?.filter((list) => {
      return (
  ...
  useEffect(() => {
    setStructureList(structuresList);
  }, [structuresList]);
  ...
  const handleSearch = (e) => {
    ...
    setSearchName(name);
    const filterdList = structuresList?.filter((list) => {
  ...
  return (
    <div className="MachineList">
      ...
      <div className={styles.SearchContainer}>
        ...
        <div className={styles.StructureFilter}>
          {/* <h4> 建物類型:</h4> */}
          <div
            className={`${styles.FilterIcon} ${
              activeIcon.bridge ? styles.FilterIconActive : ""
            }`}
          ...
          <div
            className={`${styles.FilterIcon} ${
              activeIcon.building ? styles.FilterIconActive : ""
            }`}
          ...
          <div
            className={`${styles.FilterIcon} ${
              activeIcon.neighbor ? styles.FilterIconActive : ""
            }`}
      ...
      <div className={styles.Table}>
        <TableContainer className={styles.TableClass}>
                  ...
                  >
                    ...
                    >
                      ...
                      <CustomTooltip
                        title={
                          <div>
                            雨量: {structure.station.detail.precipitation} mm
                            <br />
                            溫度: {structure.station.detail.temperature} °C
                            <br />
                            風速: {structure.station.detail.windSpeed} m/s <br />
                            風向: {structure.station.detail.windDirection} °
                          </div>
                        }
                      >
                        <h4>{structure.station.name}</h4>
                      </CustomTooltip>
                    </TableCell>
                    <TableCell
                      sx={{ border: "1px solid #E95098" }}
                    ...
                    >
                      <h4 style={{ color: "rgb(255, 59, 59)" }}>
                        {structure.anomaly_sensor_list.counts} 顆
                      </h4>
                    </TableCell>
                    <TableCell
                      sx={{ border: "1px solid #E95098" }}
                    ...
                    >
                      <h4>
                        {structure.latest_health === null
                          ? "無資料"
                          : `${structure.latest_health}%`}
                      </h4>
                    </TableCell>
                    <TableCell
                      sx={{ border: "1px solid #E95098" }}
